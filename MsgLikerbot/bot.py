        ...
        try:
            ...
            if text.strip() == "/like" or text.strip() == "/like@MsgLikebot" and "reply_to_message" in update.get(
                ...
                if "photo" in bool_check:
                    reply_to_message("A {} le ha gustado esta foto".format(user), chat, replied_text_message_id,
                                     message_id)
                elif "sticker" in bool_check:
                    reply_to_message("A {} le ha gustado este Sticker".format(user), chat, replied_text_message_id,
                                     message_id)
                elif "video" in bool_check:
                    reply_to_message("A {} le ha gustado este video".format(user), chat, replied_text_message_id,
                                     message_id)
                elif "audio" in bool_check:
                    reply_to_message("A {} le ha gustado esta pista de audio".format(user), chat,
                                     replied_text_message_id, message_id)
                elif "animation" in bool_check:
                    reply_to_message("A {} le ha gustado este GIF".format(user), chat, replied_text_message_id,
                                     message_id)
                elif "voice" in bool_check:
                    reply_to_message("A {} le ha gustado este audio".format(user), chat, replied_text_message_id,
                                     message_id)
                elif "voice_note" in bool_check:
                    reply_to_message("A {} le ha gustado este videomensaje".format(user), chat, replied_text_message_id,
                                     message_id)
                elif "document" in bool_check:
                    reply_to_message("A {} le ha gustado este documento".format(user), chat, replied_text_message_id,
                                     message_id)
                elif "contact" in bool_check:
                    reply_to_message("A {} le ha gustado este contacto".format(user), chat, replied_text_message_id,
                                     message_id)
                elif "text" not in bool_check:
                    reply_to_message("A {} le ha gustado esta encuesta".format(user), chat, replied_text_message_id,
                                     message_id)
                else:
